...
class GISDB():
    ...
    def ClickListener(self, button, PixelPos, LogicalPos):
        if not PyScreen.CheckInRegion(LogicalPos[0], LogicalPos[1], self.LogicalLeft, self.LogicalRight, self.LogicalUp,
                                      self.LogicalDown): return
        if (not button == 1) and (not button == 3): return
        for LogListener in self.LogBoxListener:
            if not PyScreen.CheckInRegion(LogicalPos[0], LogicalPos[1], LogListener[0], LogListener[1], LogListener[2],
                                          LogListener[3]): continue
            LogListener[4](LogicalPos[0], LogicalPos[1], button)

        for PixListener in self.PixBoxListener:
            if not PyScreen.CheckInRegion(PixelPos[0], PixelPos[1], PixListener[0], PixListener[1], PixListener[2],
                                          PixListener[3]): continue
    ...
    def ClearListener(self):
        ...
        pygame.draw.rect(self.ScreenItem.screen, self.ScreenItem.SCREEN_DEFAULT_COLOR,
                         pygame.Rect(fill_pos, fill_scale))
    ...
    def draw(self):
        self.ClearListener()
        self.ScreenItem.DrawLogicalRect(self.LogicalLeft, self.LogicalRight, self.LogicalUp, self.LogicalDown,
                                        self.default_rgb)
        self.DataLock.read_acquire()
        try:
            DrawCount = 0
            for item in self.GISData:
                ...
                if (item.X0 > self.LogicalRight) or (
                        item.X0 > self.ScreenItem.LOGICAL_DEFAULT_P0[0] + self.ScreenItem.LOGICAL_DEFAULT_SIZE[
                    0]): continue
                if (item.Y1 < self.LogicalDown) or (item.Y1 < self.ScreenItem.LOGICAL_DEFAULT_P0[1]): continue
                if (item.Y0 > self.LogicalUp) or (
                        item.Y0 > self.ScreenItem.LOGICAL_DEFAULT_P0[1] + self.ScreenItem.LOGICAL_DEFAULT_SIZE[
                    1]): continue
                DrawCount += 1
    ...
    def test(self):
        ...
        while self.ScreenItem.running:
            ...
            for i in range(10):
                ...
                self.ScreenItem.DrawLogicalPoint(LogicalPoint=(
                    random.randint(self.LogicalLeft, self.LogicalRight),
                    random.randint(self.LogicalDown, self.LogicalUp)),
                    rgb=rect_color, radius=2)
                p1 = (random.randint(self.LogicalLeft, self.LogicalRight - 80),
                      random.randint(self.LogicalDown, self.LogicalUp - 80))
                p2 = (p1[0] + 12, p1[1] + 36)
                p3 = (p2[0] + 31, p2[1] + 17)
            ...
            text = pygame.transform.scale(self.ScreenItem.font.render(u'鼠标点击', 1, (255, 0, 0)),
                                          self.ScreenItem.LogSize2Pixel((70, 16)))  # button1
            self.ScreenItem.screen.blit(text, self.ScreenItem.Log2Pix((self.LogicalLeft, self.LogicalUp)))
            self.ScreenItem.DrawLogicalRect(self.LogicalLeft, self.LogicalLeft + 70, self.LogicalUp,
                                            self.LogicalUp - 16)
            icon = pygame.transform.scale(pygame.image.load("plane.jpg"),
                                          self.ScreenItem.LogSize2Pixel((32, 32)))  # button2
            self.ScreenItem.screen.blit(icon, self.ScreenItem.Log2Pix((self.LogicalLeft, self.LogicalDown + 32)))
            self.ScreenItem.DrawLogicalRect(self.LogicalLeft, self.LogicalLeft + 32, self.LogicalDown + 32,
                                            self.LogicalDown)
